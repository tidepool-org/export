...
function getQueryOptions(params) {
  ...
  if (dates && dates.start && dates.end) {
    options.startDate = moment.utc(dates.start).toISOString();
    options.endDate = moment.utc(dates.end).toISOString();
  } else {
...
function getDateRangeByReport(
  ...
  timezoneName,
  {
    agpDays = 14, basicsDays = 14, dailyDays = 14, bgLogDays = 30,
  } = {},
) {
