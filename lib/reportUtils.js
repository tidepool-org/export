 ...
 *  }} queries
 */
function buildReportQueries(params) {
  const { bgUnits = mgdLUnits, timePrefs, reports = [reportTypes.all] } = params;
  const reportBGPrefs = getBGPrefs(bgUnits);
  const dataQueries = {
    basics: {
      endpoints: [],
 ...
 * }}
 */
function getUserDataQueryOptions(params) {
  const {
    data, bgUnits, dates, serverTime, restrictedToken,
  } = params;

  const options = {
    initial: true,
    bgPrefs: getBGPrefs(bgUnits),
  };

  if (dates && dates.startDate && dates.endDate) {
    let start = moment.utc(dates.startDate);
    const end = moment.utc(dates.endDate);
...
function getDateRangeByReport(params) {
  ...
  if (reportDates && reportDates.startDate && reportDates.endDate) {
    ...
    let bgLogStartDate = moment(reportDates.startDate).tz(timezoneName).startOf('day');
 ...
 */
function runReportQueries(queries, opts, dataUtil) {
...
function getReportOptions(params) {
  ...
  const reportQueries = buildReportQueries({
    units: bgUnits,
    timePrefs,
    reports,
  });
...
async function generateReport(
  ...
  let userDataQueryParams;
  let timePrefs;

  // if given dates we use those
  if (startDate && endDate) {
    userDataQueryParams = getUserDataQueryOptions({
      bgUnits,
  ...
  log.debug('data query options ', userDataQueryParams);
  ...
  requestConfig.params = userDataQueryParams;
  ...
  const reportData = runReportQueries(queries, printOptions, dataUtil);
