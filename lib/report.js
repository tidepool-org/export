...
class Report {
  ...
  async generate() {
    ...
    this.#log.debug('queries=', JSON.stringify(queries));
    this.#log.debug('print options', JSON.stringify(printOptions));

    const reportData = this.runReportQueries(queries, printOptions);

    this.#log.debug('getting pdf report types');
    ...
    this.#log.debug('processing pdf report types');
    ...
    this.#log.debug('creating print pdf package');
